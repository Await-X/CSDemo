...
//static const CGFloat   kDandaoCount  = 5.0;
static int kDandaoCount  = 5;
...
@property (nonatomic, strong) NSMutableArray *danmakuModelArr;
...
- (void)tapClick:(UITapGestureRecognizer *)tap {
    ...
    for (UIView *danmuView in self.danmakuViewArr) {
        ...
        if (isContain) {
            NSInteger index = [self.danmakuViewArr indexOfObject:danmuView];
            if ([self.delegate respondsToSelector:@selector(danmuViewDidClick:at:model:)]) {
                [self.delegate danmuViewDidClick:danmuView at:point model:self.danmakuModelArr[index]];
            }
            break;
        }
    }
...
- (void)setKDandaoCountSs:(int)kDandaoCountSs
{
    _kDandaoCountSs = kDandaoCountSs;
    
    kDandaoCount = kDandaoCountSs;
}
...
- (BOOL)checkBoomAndBiuWith:(id<FDanmakuModelProtocol>)model {
    ...
    for (int i = 0; i < kDandaoCount; i++) {
        ...
        [self.danmakuModelArr addObject:model];
        ...
        [UIView animateWithDuration:model.liveTime delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            [self.danmakuModelArr removeObject:model];
        }];
...
- (NSMutableArray *)danmakuModelArr {
    if (!_danmakuModelArr) {
        _danmakuModelArr = [NSMutableArray array];
    }
    return  _danmakuModelArr;
}
