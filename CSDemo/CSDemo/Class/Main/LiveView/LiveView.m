...
#define DefaultToolButtonWidth 35.0

@interface LiveView () <CAAnimationDelegate>
///
...
- (void)layoutSubviews
{
    ...
    self.btn.frame = CGRectMake(0, self.height - DefaultToolButtonWidth, DefaultToolButtonWidth, DefaultToolButtonWidth);
}
...
//    imageView.centerX = self.width * .5f;
    ...
    maskLayer.frame = CGRectMake(0, self.height - 30, 30, 30);
    maskLayer.contents = (__bridge id)[self randomLikeImage].CGImage;
    
    CALayer *transitionLayer = maskLayer;//imageView.layer;
    ...
    [movePath addQuadCurveToPoint:toPoint controlPoint:controlPonit];
    XPFLog(@" ----- %f, %f", toOffsetX, toOffsetY);
...
- (void)btnAction:(UIButton *)btn
{
    ...
    // 2021-03-17 17:28:21  改需求 可以连续点击，
...
//    //btn.enabled = YES;
//
//    if (_delegate && [_delegate respondsToSelector:@selector(likeViewSendZan:isContinuityClick:)])
//    {
//        [_delegate likeViewSendZan:self isContinuityClick:NO];
//    }
//}
...
- (UIButton *)btn
{
    if (!_btn) {
        ...
        [_btn setImage:[UIImage imageNamed:@"lufei"] forState:UIControlStateNormal];
        [_btn setImage:[UIImage imageNamed:@"lufei"] forState:UIControlStateHighlighted];
