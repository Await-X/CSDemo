...
#define kAdeleImage @"删除.png" // 删除按钮图片
#define kAddImage @"添加.png" // 添加按钮图片

#import "XPFAddImage.h"

@interface XPFAddImage ()
<
...
//        [self addSubview:btn];
        UIView *view = [self createView_ButtonWithImage:kAddImage andSeletor:@selector(addNew:) is:NO anddele:nil];
...
- (UIView *)createView_ButtonWithImage:(id)imageNameOrImage andSeletor:(SEL)selector is:(BOOL)isok anddele:(SEL)deleSelector
{
    UIView *addView = nil;
    ...
    */
    ...
    if (isok) {
        ...
        [deleBtn setBackgroundImage:addImage forState:(UIControlStateNormal)];
        [deleBtn addTarget:self action:deleSelector forControlEvents:UIControlEventTouchUpInside];
        deleBtn.tag = self.subviews.count + 1;
    }
...
//        [self insertSubview:btn atIndex:self.subviews.count - 1];
        ...
        UIView *view = [self createView_ButtonWithImage:kAddImage andSeletor:@selector(changeOld:) is:YES anddele:@selector(deletePic:)];
