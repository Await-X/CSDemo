...
@implementation PageControlTool
...
- (void)updateUI
{
    ...
    if (self.isInitialize)
    {
        ...
        for (int i = 0; i < self.dotViewArrayM.count; i++) {
            ...
            [self modifyView:dotView page:i];
...
- (void)setNumberOfPages:(int)numberOfPages
{
    ...
    for (int i = 0; i < numberOfPages; i++) {
        ...
        [self modifyView:dotView page:numberOfPages];
...
- (void)setCurrentPage:(int)currentPage
{
    ...
    if (currentPage > self.currentPage)
    {
        ...
        [self modifyView:currentView page:currentPage];
...
- (void)modifyView:(UIView *)currentView page:(int)currentPage
{
    CGRect tempFrame = currentView.frame;
    // 当前选中的圆点，x 不变，宽度增加，增加几个圆点和间隙距离
    CGFloat width = self.currentDotWidth + (self.dotSpace + self.otherDotWidth) * (currentPage - self.currentPage);
    ...
    currentView.layer.borderWidth = 4.0f;
}
