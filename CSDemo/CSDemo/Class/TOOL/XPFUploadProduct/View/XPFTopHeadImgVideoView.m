...
@interface XPFTopHeadImgVideoView ()
<
HXPhotoViewDelegate
>
{
    NSUInteger SelectPhotoMaxNum;
    NSUInteger SelectVideoMaxNum;
}
///
...
@property (copy, nonatomic) NSMutableArray *imageRequestIds;
@property (copy, nonatomic) NSMutableArray *videoSessions;
///
@property (assign, nonatomic) BOOL original;

@end

@implementation XPFTopHeadImgVideoView

- (instancetype)initWithFrame:(CGRect)frame SelectImgMax:(CGFloat)ImgMax SelectVideoMax:(CGFloat)VideoMax
{
    self = [super initWithFrame:frame];
    if (self) {
        SelectPhotoMaxNum = ImgMax;
        SelectVideoMaxNum = VideoMax;
        [self changeStatus];
...
- (HXPhotoManager *)manager {
    if (!_manager) {
        ...
        _manager.configuration.photoMaxNum = SelectPhotoMaxNum;
        _manager.configuration.videoMaxNum = SelectVideoMaxNum;
        _manager.configuration.videoMaximumDuration = 60;
        _manager.configuration.maxNum = SelectPhotoMaxNum + SelectVideoMaxNum;
...
- (void)addSubContentView {
    ...
    //self.automaticallyAdjustsScrollViewInsets = YES; // 导航有关
...
//
//}
- (void)didNavOneBtnClickBlock:(void (^)(BOOL iscompletion))completionBlock
{
//    self.imageRequestIds = [[NSMutableArray array] init];
//    self.videoSessions   = [[NSMutableArray array] init];
            ...
            else
            {
                ...
                if (model.type == HXPhotoModelMediaTypePhoto)
                {
                    ...
                    [model requestPreviewImageWithSize:size startRequestICloud:^(PHImageRequestID iCloudRequestId, HXPhotoModel * _Nullable model) {
                        ...
                        // image
                        [self.imageRequestIds addObject:image];
                        completionBlock(YES);
        ...
        else if (model.subType == HXPhotoModelMediaSubTypeVideo)
        {
            ...
            if (model.type == HXPhotoModelMediaTypeVideo)
            {
                ...
                [model exportVideoWithPresetName:AVAssetExportPresetMediumQuality startRequestICloud:nil iCloudProgressHandler:nil exportProgressHandler:^(float progress, HXPhotoModel * _Nullable model) {
                    ...
                    // 导出完成, videoURL
                    [self.videoSessions addObject:model.videoURL];
                    completionBlock(YES);
                } failed:nil];
...
- (NSMutableArray *)imageRequestIds
{
    if (!_imageRequestIds) {
        _imageRequestIds = [NSMutableArray array];
    }
...
- (void)photoView:(HXPhotoView *)photoView changeComplete:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photos videos:(NSArray<HXPhotoModel *> *)videos original:(BOOL)isOriginal {
    ...
    [self didNavOneBtnClickBlock:^(BOOL iscompletion) {
        if (iscompletion)
        {
            //
            if (self.delegateImgVideo && [self.delegateImgVideo respondsToSelector:@selector(onTopHeadImgAry:video:)]) {
                //[self.delegateImgVideo onTopHeadImgVideoAry:self.selectList];
                [self.delegateImgVideo onTopHeadImgAry:self.imageRequestIds video:self.videoSessions];
            }
        }
    }];
}
...
- (void)photoView:(HXPhotoView *)photoView updateFrame:(CGRect)frame {
    NSSLog(@"photoView===updateFrame: %@",NSStringFromCGRect(frame));
    //
    self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, CGRectGetMaxY(frame) + kPhotoViewMargin);
    //
    self.scrollView.frame = CGRectMake(0, 0, self.scrollView.frame.size.width, (CGRectGetMaxY(frame) + kPhotoViewMargin));
    
    NSSLog(@"photoView===updateFrame: %@, %f, %f", NSStringFromCGRect(frame), CGRectGetMaxY(frame) + kPhotoViewMargin, CGRectGetMaxY(frame));
    //
    if (self.delegateImgVideo && [self.delegateImgVideo respondsToSelector:@selector(onTopHeadImgVideoViewH:)]) {
        [self.delegateImgVideo onTopHeadImgVideoViewH:(CGRectGetMaxY(frame) + kPhotoViewMargin)];
    }
}
